...
INSERT INTO saticilar
    (satici_id, satici_adi, email, sehir, katilim_tarihi, durum, toplam_satis, ortalama_puan)
    VALUES
    (1, 'Teknoloji Plus', 'tekno@shophub.com', 'Istanbul', '2020-01-15', 'Aktif', 15420, 4.8),
    (2, 'Fashion World', 'fashion@shophub.com', 'Istanbul', '2020-03-20', 'Aktif', 8950, 4.5),
    (3, 'Home Comfort', 'home@shophub.com', 'Ankara', '2020-06-10', 'Aktif', 6780, 4.6),
    (4, 'Sports Zone', 'sports@shophub.com', 'Izmir', '2021-01-05', 'Aktif', 4320, 4.3),
    (5, 'Book World', 'books@shophub.com', 'Ankara', '2021-02-14', 'Aktif', 3240, 4.7),
    (6, 'Toy Paradise', 'toys@shophub.com', 'Istanbul', '2021-04-22', 'Aktif', 2890, 4.4),
    (7, 'Beauty Experts', 'beauty@shophub.com', 'Antalya', '2021-05-30', 'Aktif', 5670, 4.9),
    (8, 'Auto Parts Plus', 'auto@shophub.com', 'Bursa', '2021-07-12', 'Aktif', 3450, 4.2),
    (9, 'Gourmet Foods', 'food@shophub.com', 'Gaziantep', '2021-08-18', 'Aktif', 2100, 4.6),
    (10, 'Health Plus', 'health@shophub.com', 'Istanbul', '2021-09-25', 'Aktif', 4560, 4.5),
    (11, 'Premium Electronics', 'premium@shophub.com', 'Istanbul', '2020-02-01', 'Aktif', 22340, 4.9),
    (12, 'Mega Store', 'mega@shophub.com', 'Ankara', '2022-01-10', 'Beklemede', 0, 0.0);
...
INSERT INTO musteriler
    (musteri_id, ad_soyad, email, telefon, sehir, adres, kayit_tarihi, uyelik_tipi, toplam_harcama, siparis_sayisi)
    VALUES
...
INSERT INTO urunler
    (urun_id, urun_adi, kategori_id, satici_id, fiyat, indirimli_fiyat, stok_sayisi, aciklama, ortalama_puan, yorum_sayisi, satilan_adet)
    VALUES
...
INSERT INTO siparisler
    (siparis_id,musteri_id,siparis_tarihi,toplam_tutar,indirim_tutari,son_tutar,durum)
    VALUES
...
INSERT INTO siparis_detaylari (detay_id,siparis_id,urun_id,miktar,birim_fiyati,alt_toplam)
    VALUES
    ...
    (69, 19, 19, 1, 8000.00, 8000.009),
    (70, 20, 16, 1, 2200.00, 2200.00);

CREATE TABLE odemeler(
    odeme_id SERIAL PRIMARY KEY,
    sipris_id INTEGER NOT NULL REFERENCES siparisler(siparis_id),
...
INSERT INTO odemeler
    (odeme_id, sipris_id, odeme_tipi, tutar, odeme_tarihi, durum)
    VALUES
...
INSERT INTO kargo_sirketleri(kargo_id,sirket_adi,telefon,web_sitesi,ortalama_teslimat_gunu)
    VALUES
...
INSERT INTO kargo_tracking
    (tracking_id, siparis_id, kargo_id, takip_numarasi, kargo_tarihi, teslimat_tarihi, durum)
    VALUES
...
INSERT INTO yorumlar (yorum_id, urun_id,musteri_id,puan,yorum_metni,yorum_tarihi,yardimci_sayisi)
    VALUES
...
INSERT INTO promosyonlar
    (promo_id, promo_kodu, aciklama, indirim_orani, baslama_tarihi, bitis_tarihi, kullanim_sayisi, maksimum_kullani)
    VALUES
...
INSERT INTO favoriler(favori_id,musteri_id,urun_id,eklenme_tarihi)
    VALUES
...
INSERT INTO indirimler
    (indirim_id, urun_id, orijinal_fiyat, indirimli_fiyat, indirim_yuzde, baslama_tarihi, bitis_tarihi)
    VALUES
...
-- Tüm ürünlerin adlarını ve fiyatlarını göster.

SELECT urun_adi,fiyat
FROM urunler

-- Kategorisi 'Elektronik' olan ürünlerin adlarını ve stok sayılarını göster.
SELECT
    u.urun_adi,
    u.stok_sayisi,
    k.kategori_adi
...
-- Müşterilerin adlarını ve email adreslerini göster, ada göre A'dan Z'ye sırala.
SELECT ad_soyad, email
FROM musteriler
ORDER BY ad_soyad ASC;

-- Durum değeri 'Teslim Edildi' olan tüm siparişleri göster.
SELECT * FROM siparisler
WHERE durum = 'Teslim Edildi';

-- Satıcılar tablosundan şu andaki 'Aktif' satıcıları listele.
SELECT satici_adi,durum
FROM saticilar
WHERE durum ='Aktif';

-- Ürün id 1, 2, 3, 4, 5 olan ürünlerin adları ve fiyatlarını göster.
SELECT urun_id, urun_adi, fiyat
FROM urunler
WHERE urun_id BETWEEN 1 AND 5;

-- 50.000 TL'den fazla fiyata sahip ürünleri göster.
SELECT urun_adi,fiyat
FROM urunler
WHERE fiyat > 50000;

-- 2024-01-20'den sonra yapılan siparişleri göster.
SELECT siparis_id, siparis_tarihi
FROM siparisler
WHERE siparis_tarihi > '2024-01-20';

-- Başarılı ödemeler ('Basarili') tablosunu göster.
SELECT odeme_id, odeme_tipi,durum
FROM odemeler
WHERE durum = 'Basarili';

-- Ortalama puanı 4.5 ve üzeri olan ürünleri göster.
SELECT urun_adi,ortalama_puan
FROM urunler
...
-- Müşteri adı 'Ayşe Yilmaz' olan kişinin tüm siparişlerini göster (siparis_id, toplam_tutar, durum).
SELECT
    m.ad_soyad,
    s.siparis_id,
    s.toplam_tutar,
...
-- Hangi müşteri en çok para harcadı? Müşteri adını ve toplam harcama tutarını göster.
SELECT ad_soyad, toplam_harcama
FROM musteriler
ORDER BY toplam_harcama DESC
LIMIT 1;

-- En çok satılan 5 ürünü göster (ürün adı ve satılan adet)
...
-- Sipariş 1'in detaylarını göster (hangi ürünler, kaç tane, fiyat).
SELECT
    sd.siparis_id,
    u.urun_adi,
    sd.miktar,
...
-- Toplam siparişin 5'ten fazla olan müşterileri göster.
SELECT
    m.ad_soyad,
...
HAVING SUM(sd.miktar) > 5
ORDER BY toplam_miktar DESC;

-- Gold' üyelik türü olan müşterilerin adlarını ve toplam harcamalarını göster.
SELECT ad_soyad,uyelik_tipi,toplam_harcama
FROM musteriler
WHERE uyelik_tipi ='Gold';

-- Her satıcıdan kaç tane ürün satılmış? (Satıcı adı, satılan ürün sayısı)
SELECT
    s.satici_adi,
    u.satilan_adet
FROM urunler u
JOIN saticilar s
    ON u.satici_id = s.satici_id;

-- 4 ve üzeri puanı olan ürünlerin ortalamasını hesapla.
SELECT
    AVG(ortalama_puan) AS ortalama_puan
FROM urunler
WHERE ortalama_puan >= 4;

-- Kargo durumu 'Teslim Edildi' olan kaç sipariş var?
SELECT
    COUNT(s.siparis_id) AS teslim_edilen_siparis_sayisi
FROM kargo_tracking kt
JOIN siparisler s
    ON kt.siparis_id = s.siparis_id
WHERE kt.durum = 'Teslim Edildi';

-- Müşteri 'Fatma Sahin'in favorilerine eklediği ürünleri göster.
SELECT
    m.ad_soyad,
    f.favori_id,
    u.urun_adi
FROM musteriler m
LEFT JOIN favoriler f
    ON f.musteri_id = m.musteri_id
LEFT JOIN urunler u
    ON u.urun_id =f.urun_id
WHERE m.ad_soyad ='Fatma Sahin';

-- Promosyon kodu 'WELCOME10'un kaç kez kullanıldığını göster.
SELECT
    COUNT(promo_kodu) AS toplam_kullanim_kodu
FROM promosyonlar
WHERE promo_kodu ='WELCOME10';

-- Yorum puanı 5 olan ürünlerin adlarını ve kaç tane 5 yorum aldığını göster.
SELECT
    u.urun_adi,
    COUNT(y.yorum_id) AS bes_puanli_yorum_sayisi
FROM yorumlar y
JOIN urunler u
...
-- İndirim yüzdesi en yüksek 3 ürünü göster.
SELECT
    u.urun_adi,
    i.indirim_yuzde
FROM indirimler i
JOIN urunler u
    ON i.urun_id=u.urun_id
ORDER BY i.indirim_yuzde DESC
LIMIT 3;

-- 'Yolda' durumundaki kargo takiplerine ait siparişleri göster.
SELECT
    kt.durum,
    s.siparis_id
FROM kargo_tracking kt
...
-- Müşteri 'Mehmet Kaya' hangi ürünleri aldı, kargo durumu nedir, kaç TL harcadı?
SELECT
    m.ad_soyad,
    m.toplam_harcama,
    u.urun_adi,
...
JOIN urunler u
    ON sd.urun_id = u.urun_id
JOIN kargo_tracking kt
    ON s.siparis_id = kt.siparis_id
WHERE m.ad_soyad = 'Mehmet Kaya';

-- En iyi puanlanmış satıcının adını ve satıcı puanını göster.
SELECT satici_adi,ortalama_puan
FROM saticilar
ORDER BY ortalama_puan DESC
LIMIT 1;

-- Toplam satış değeri en yüksek 5 satıcıyı göster (satıcı adı, toplam satış tutar).
SELECT satici_adi,toplam_satis
FROM saticilar
ORDER BY toplam_satis DESC
LIMIT 5;

-- Her müşteriye kaç sipariş yapmış, toplam ne harcamış göster, en çok harcayandan az harcayan'a sırala.
SELECT
    m.ad_soyad,
    m.toplam_harcama,
...
JOIN musteriler m
    ON m.musteri_id = s.musteri_id
GROUP BY m.musteri_id, m.ad_soyad, m.toplam_harcama
ORDER BY m.toplam_harcama DESC;

-- Elektronik kategorisindeki ürünlerin ortalama fiyatı nedir? Stok sayısı kaçtır?
SELECT
    k.kategori_id,
...
-- Sipariş tarihine göre grouplandırılmış, her gün kaç sipariş alındığını göster.
SELECT
    siparis_tarihi,
    COUNT(siparis_id) AS alinan_siparis
FROM siparisler
GROUP BY siparis_tarihi
ORDER BY siparis_tarihi;

-- En çok yorum alan 5 ürünü göster (ürün adı, yorum sayısı, ortalama puan)
SELECT urun_adi, yorum_sayisi,ortalama_puan
FROM urunler
ORDER BY yorum_sayisi DESC
LIMIT 5;

-- Her kargo şirketinin ortalama kaç gün'de teslim ettiğini hesapla.
SELECT
    sirket_adi,
    ortalama_teslimat_gunu
FROM kargo_sirketleri
ORDER BY ortalama_teslimat_gunu ASC;

-- Promosyonlar tablosundan, başlama tarihi 2024-01-01'den sonra olan ve indirim yüzdesi 15% üzeri olan promosyonları göster.
SELECT promo_kodu, baslama_tarihi, indirim_orani
FROM promosyonlar
WHERE baslama_tarihi > '2024-01-01'
    AND indirim_orani > 15.00;

-- Müşteri 1'in tüm satın aldığı ürünleri göster (ürün adı, kategori, fiyat, toplam). siparisler tablosundan birleştirerek.
SELECT
    m.musteri_id,
    k.kategori_adi,
    u.urun_adi,
...
JOIN kategoriler k
    ON k.kategori_id = u.kategori_id
WHERE m.musteri_id = 1;

---------------------ÇOK İLERİ SEVİYE SORULAR (⭐⭐⭐⭐)-----------------------------------------------

-- Her müşterinin son 3 ayda yaptığı siparişlerin ortalamasını hesapla. (Müşteri adı, sipariş ortalaması)
SELECT
    m.ad_soyad,
    AVG(s.siparis_sayisi) AS ortalama_siparis_sayisi
FROM musteriler m
JOIN siparisler s
    ON s.musteri_id = m.musteri_id
...
-- Aynı kategorideki ürünlerin ortalama fiyatlarından daha yüksek fiyata sahip ürünleri göster.
SELECT
    u.urun_adi,
    u.fiyat,
    k.kategori_adi
...
WHERE u.fiyat > (
    SELECT AVG(u2.fiyat)
    FROM urunler u2
    WHERE u2.kategori_id = u.kategori_id
);

-- Toplam harammı değeri en yüksek 3 müşterinin aldığı ürünler nelerdir? (Müşteri adı, ürün adı, harcadığı tutar)
SELECT
    m.ad_soyad,
    u.urun_adi,
    sd.miktar * sd.birim_fiyati AS harcadigi_tutar
FROM musteriler m
JOIN siparisler s
    ON m.musteri_id = s.musteri_id
JOIN siparis_detaylari sd
